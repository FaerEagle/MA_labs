...
def authenticate_tokenAPIView(request):  # обработка кода
    ...
    token_queryset = UserProfile.objects.filter(
        token=token)  # Profile это как у тебя модель с пользователем и токеном называться будет. Можешь хоть как назвать.

    if token_queryset.exists():
        profile = token_queryset[0]

        if profile.token == token and (token != None):
